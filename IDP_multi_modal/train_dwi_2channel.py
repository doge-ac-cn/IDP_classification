...
max_epoch = 300
data_path = 'dataset_dwi_2channel'
    ...
    for train_index, test_index in kfold.split(images_all):
        ...
        train_transforms = Compose(
            [
                ...
                # 裁剪出指定大小的区域
                CenterSpatialCropd(keys=["img"], roi_size=(256, 192, 32)),
                
                # 使用 SpatialPadd 将图像填充到最小尺寸 (64, 64, 32)
                SpatialPadd(keys=["img"], spatial_size=(256, 192, 32)),
            ]
        )
        ...
        torch.save(model.state_dict(), f"pth_2channel/folder{folder_index}.pth")
        print("saved model")
        folder_index += 1
        writer.close()
