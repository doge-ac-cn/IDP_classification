...
max_epoch = 300
data_path = f'dataset_dwi_{channel_num}channel'
...
def main():
    ...
    labels_all = [int(os.path.basename(img)[0]) for img in images_all]
    ...
    for train_index, test_index in kfold.split(images_all):
        ...
        train_transforms = Compose([
            ...
            RandAffined(
                keys=["img"],
                prob=1.0,
                shear_range=(0.1, 0.1, 0.1 if channel_num ==2 else 0),
                translate_range=(0.1, 0.1, 0.1 if channel_num ==2 else 0),
                # rotate_range=(0.1, 0.1, 0.1 if channel_num ==2 else 0),
                scale_range=(0.1, 0.1, 0.1 if channel_num ==2 else 0),
                padding_mode="zeros",
            ),
            # # 随机加入高斯噪声
            ...
            CenterSpatialCropd(keys=["img"], roi_size=(256, 192, 64//channel_num)),
            SpatialPadd(keys=["img"], spatial_size=(256, 192, 64//channel_num)),
        ])
        ...
        nib.save(nii_img, f"check_image_{channel_num}channel.nii")
        ...
        model = monai.networks.nets.DenseNet121(spatial_dims=3, in_channels=channel_num, out_channels=2).to(device)
        # model = monai.networks.nets.resnet50(spatial_dims=3, n_input_channels=channel_num, num_classes=2).to(device)
        # model = monai.networks.nets.EfficientNetBN("efficientnet-b0",spatial_dims=3, in_channels=channel_num, num_classes=2).to(device)
        ...
        np.save(f"result/{channel_num}channel/predictions_fold{folder_index}_epoch{epoch + 1}.npy", y_pred.cpu().numpy())
        np.save(f"result/{channel_num}channel/labels_fold{folder_index}_epoch{epoch + 1}.npy", y.cpu().numpy())
        ...
        torch.save(model.state_dict(), f"pth_{channel_num}channel/folder{folder_index}.pth")
        print("saved model")
        folder_index += 1
        writer.close()
