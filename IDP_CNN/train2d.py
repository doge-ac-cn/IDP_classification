    ...
    for patient_name_train_index, patient_name_test_index in kfold.split(patients_names):
        ...
        train_transforms = Compose(
            [
                ...
                # SpatialPadd(keys=["img"], spatial_size=(256, 256)),
                # 裁剪出指定大小的区域
                CenterSpatialCropd(keys=["img"], roi_size=(192, 192)),
        ...
        val_transforms = Compose(
            [
                ...
                # SpatialPadd(keys=["img"], spatial_size=(256, 256)),
                # 裁剪出指定大小的区域
                CenterSpatialCropd(keys=["img"], roi_size=(256, 256)),
        ...
        train_loader = DataLoader(train_ds, batch_size=16, shuffle=True, num_workers=20, pin_memory=torch.cuda.is_available())

        # create a validation data loader
        val_ds = monai.data.Dataset(data=val_files, transform=val_transforms)
        val_loader = DataLoader(val_ds, batch_size=16, num_workers=20, pin_memory=torch.cuda.is_available())
