    ...
    for patient_name_train_index, patient_name_test_index in kfold.split(patients_names):
        ...
        train_transforms = Compose(
            [
                ...
                # 裁剪出指定大小的区域
                CenterSpatialCropd(keys=["img"], roi_size=(192, 192, 48)),
                
                # 使用 SpatialPadd 将图像填充到最小尺寸 (64, 64, 32)
                SpatialPadd(keys=["img"], spatial_size=(192, 192, 48)),
            ]
        )
        ...
        train_loader = DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=12, pin_memory=torch.cuda.is_available())

        # create a validation data loader
        val_ds = monai.data.Dataset(data=val_files, transform=val_transforms)
        val_loader = DataLoader(val_ds, batch_size=8, num_workers=12, pin_memory=torch.cuda.is_available())
