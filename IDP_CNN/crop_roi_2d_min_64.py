...
def ensure_minimum_size(cropped_data, x_min, x_max, y_min, y_max, required_size=128):
    """
    检查并调整裁剪后的图像大小，确保其至少为 64x64。

    参数:
    cropped_data: 原始T1C图像的裁剪区域数据
    x_min, x_max, y_min, y_max: 裁剪区域的边界
    required_size: 最小裁剪大小 (默认为64x64)

    返回:
    调整后的边界 (x_min, x_max, y_min, y_max)
    """
    width = x_max - x_min + 1
    ...
    if width < required_size:
        ...
        x_min = max(0, center_x - required_size // 2)
        x_max = x_min + required_size - 1
        x_max = min(cropped_data.shape[0] - 1, x_max)

    if height < required_size:
        center_y = (y_min + y_max) // 2
        y_min = max(0, center_y - required_size // 2)
        y_max = y_min + required_size - 1
        y_max = min(cropped_data.shape[1] - 1, y_max)
...
def crop_roi_from_image(t1c_path, roi_paths, output_dir):
    ...
    for roi_path in roi_paths:
        ...
        for i, (z_idx, area, x_min, x_max, y_min, y_max) in enumerate(z_slices_sorted):
            # 调整裁剪区域，确保最小为64x64
            x_min, x_max, y_min, y_max = ensure_minimum_size(t1c_data[:, :, z_idx], x_min, x_max, y_min, y_max)
...
output_root_dir = './cropped_2d_min128'  # 保存裁剪结果的目录
